...
export async function GET(req: Request) {
  ...
  if (fs.existsSync
    (fullRequestedPath + ".md")) {
    return NextResponse.json({ info: "Markdown file found", ok: true }, { status: 200 });
  }
  if (!fs.existsSync(fullRequestedPath) || !fs.statSync(fullRequestedPath).isDirectory()) {
  ...
  function getFileTree(dirPath: string, relativePath = ""): FileTreeNode[] {
    ...
    return items
      .filter((item) => !["_index.md", "assets", "images", "data"].includes(item.name))
      .map((item) => {
        const itemPath = path.join(relativePath, item.name);
        const filePath = path.join(dirPath, itemPath);
        const url = path.join("docs", requestedPath, itemPath).replace(/ /g, "-");

        if (item.isDirectory()) {
          const metadata = getMetadataFromMarkdown(path.join(filePath, "_index.md"));
          return {
            type: "folder",
            path: itemPath,
