...
export function searchMarkdownFiles(keyword: string, directory: string): FileTreeNode[] {
  ...
  function searchDirectory(dir: string, relativePath = "") {
    ...
    items.forEach((item) => {
      ...
      const url = itemPath.replace(".md", "").replace(/_index$/, "").replace(/ /g, "-");
      ...
      if (item.isDirectory()) {
        ...
        if (metadata.title && (regex.test(metadata.title) || regex.test(metadata.description))) {
